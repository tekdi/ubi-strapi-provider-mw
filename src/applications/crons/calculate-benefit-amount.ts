...
export class ApplicationStatusUpdate {
	...
	onModuleInit() {
		const cronExpression =
			this.configService.get('BENEFIT_CALCULATIONS_CRON_TIME') ??
			'*/30 * * * *';
		const job = new CronJob(cronExpression, () =>
			this.updateApplicationStatusCron()
		);
	...
	private async updateApplicationStatusCron() {
		try {
			...
			Logger.error(
				`Error in 'benefit calculation cron': ${error.message}`,
				error.stack
			);
		}
	}
	...
	async getApplications() {
		const dt = new Date();
		const BENEFITS_CALCULATION_LAST_PROCESS_HOURS = Number(
	...
	private async processApplications(applications: any[]) {
		for (const app of applications) {
			try {
				await this.applicationsService.calculateBenefit(Number(app.id));
			} catch (err) {
				Logger.warn(
					`Failed to update application ${app.id}: ${err.message}`
				);
			}
		}
	}
